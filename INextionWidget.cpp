 ...
 */
bool INextionWidget::setNumberProperty(char *propertyName, int32_t value)
{
  size_t commandLen = 9 +strlen(m_page)+ strlen(m_name) + strlen(propertyName);
  char commandBuffer[commandLen];
  snprintf(commandBuffer, commandLen, "%s.%s.%s=%ld",m_page, m_name, propertyName, value);
 ...
 */
int32_t INextionWidget::getNumberProperty(char *propertyName)
{
  size_t commandLen = 8 +strlen(m_page)+ strlen(m_name) + strlen(propertyName);
  char commandBuffer[commandLen];
  snprintf(commandBuffer, commandLen, "get %s.%s.%s", m_page,m_name, propertyName);
...
{
  size_t commandLen = 8+strlen(m_page) + strlen(m_name) + strlen(propertyName) + strlen(value);
  char command[commandLen];
  snprintf(command, commandLen, "%s.%s.%s=\"%s\"",m_page, m_name, propertyName, value);
                                         ...
                                         size_t len)
{
  size_t commandLen = 9+strlen(m_page)+ strlen(m_name) + strlen(propertyName);
  char command[commandLen];
  snprintf(command, commandLen, "get %s.%s.%s",m_page, m_name, propertyName);
